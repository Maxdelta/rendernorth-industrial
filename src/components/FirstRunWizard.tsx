...
export function FirstRunWizard({ onComplete }: { onComplete: () => void }) {
  ...
  return <div className="setup-shell">
    <aside className="setup-rail"><div className="setup-brand">RenderNorth <span>Industrial</span></div><div className="setup-kicker">Open Beta 0.1.3 Setup</div>{STEPS.map((label, index) => <button key={label} onClick={() => setStep(index)} className={`${index === step ? "active" : ""} ${readiness[index] ? "complete" : ""}`}><span>{readiness[index] ? "✓" : index + 1}</span>{label}</button>)}</aside>
