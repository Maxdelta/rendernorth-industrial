...
import { commerceTabAttributes } from "../lib/commerceUiState.js";
...
function KpiCard({ label, value, money, note, tone = "normal", symbol = "◆" }: { label: string; value?: string; money?: string | null; note?: string; tone?: KpiTone; symbol?: string }) {
  return <div className={`commerce-kpi ${tone}`}>
...
export function CommercePage() {
  ...
  useEffect(() => {
    ...
    return () => { active = false; };
  }, [tab, characterId, side, threshold, location, search, sort, status, contractType, availability, direction, startLocation, endLocation]);
  useEffect(() => {
    let active = true;
  ...
  return <div className="page-stack commerce-page">
    ...
    {overview&&<div className="commerce-overview">
      ...
      <div className="commerce-sync" aria-label="Commerce synchronization status"><SyncStatus label="Market Orders" state={overview.marketOrdersSync}/><SyncStatus label="Contracts" state={overview.contractsSync}/></div>
    </div>}

    {tab === "orders"&&orders&&<section id="commerce-orders-panel" role="tabpanel" aria-labelledby="commerce-orders-tab">
      <div className="commerce-summary"><KpiCard label="Active Orders" value={orders.summary.activeOrders.toLocaleString()} tone="positive" symbol="●"/><KpiCard label="Sell Orders" value={orders.summary.sellOrders.toLocaleString()} tone="positive" symbol="▲"/><KpiCard label="Buy Orders" value={orders.summary.buyOrders.toLocaleString()} tone="normal" symbol="▼"/><KpiCard label="Remaining Sell Value" money={orders.summary.remainingSellValueIsk}/><KpiCard label="Remaining Buy Commitment" money={orders.summary.remainingBuyCommitmentIsk}/><KpiCard label="Total Escrow" money={orders.summary.totalEscrowIsk} tone="neutral"/><KpiCard label="Expiring Soon" value={orders.summary.expiringSoon.toLocaleString()} note={`Within ${threshold} day${threshold === 1 ? "" : "s"}`} tone="warning" symbol="!"/></div>
    ...
    {tab === "contracts"&&contracts&&<section id="commerce-contracts-panel" role="tabpanel" aria-labelledby="commerce-contracts-tab">
      <div className="commerce-summary"><KpiCard label="Outstanding" value={contracts.summary.outstandingContracts.toLocaleString()} tone="positive" symbol="●"/><KpiCard label="Assigned to Me" value={contracts.summary.assignedToMe.toLocaleString()}/><KpiCard label="Issued by Me" value={contracts.summary.issuedByMe.toLocaleString()}/><KpiCard label="In Progress" value={contracts.summary.inProgress.toLocaleString()} tone="positive" symbol="▶"/><KpiCard label="Expiring Soon" value={contracts.summary.expiringSoon.toLocaleString()} tone="warning" symbol="!"/><KpiCard label="Finished" value={contracts.summary.completedOrFinished.toLocaleString()} tone="neutral" symbol="✓"/><KpiCard label="Collateral Exposure" money={contracts.summary.totalCollateralExposureIsk} tone="risk" symbol="!"/><KpiCard label="Outstanding Rewards" money={contracts.summary.outstandingRewardsIsk} tone="positive"/><KpiCard label="Open Contract Value" money={contracts.summary.outstandingContractValueIsk}/></div>
