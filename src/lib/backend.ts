...
export interface InventorySearchInput { text?:string|null; assetScope?:"personal"|"corporation"|"both"|null; character?:string|null; source?:string|null; category?:string|null; group?:string|null; region?:string|null; system?:string|null; location?:string|null; resolved?:boolean|null; positiveOnly?:boolean|null; sort?:string|null; }
export interface InventorySearchRow { stackKey:string; typeId:number; typeName:string; quantity:number; unitVolumeM3:number|null; stackVolumeM3:number|null; ownerType:"Personal"|"Corporation"; owner:string; division:string|null; source:string; groupName:string|null; categoryName:string|null; locationName:string; systemName:string|null; constellationName:string|null; regionName:string|null; containerPath:string[]; locationFlag:string|null; resolved:boolean; lastSynced:string; quote:MarketQuote; }
...
export interface MarketOrderHistoryRow { orderId:number; characterId:number; characterName:string; typeId:number; itemName:string; side:string; locationId:number; locationName:string; solarSystemName:string|null; regionName:string|null; priceIsk:string; volumeTotal:number; volumeRemain:number; completedQuantity:number; completionLabel:string; esiState:"cancelled"|"expired"; issuedAt:string; expiresAt:string; firstSeenAt:string; lastObservedAt:string; seen:boolean; source:string; }
export interface MarketOrderHistorySummary { newOrders:number; completedToday:number; completedThisWeek:number; cancelled:number; expired:number; }
export interface MarketOrderHistoryDashboard { rows:MarketOrderHistoryRow[]; summary:MarketOrderHistorySummary; }
...
export interface ContractActivityMetrics { outstandingContracts:number; completedToday:number; cancelledToday:number; expiredToday:number; contractsAwaitingAcceptance:number; newContractActivity:number; }
export interface ContractDashboard { rows:ContractRow[]; summary:ContractSummary; metrics:ContractActivityMetrics; }
...
export interface CommerceOverview { remainingSellValueIsk:string; remainingBuyCommitmentIsk:string; outstandingContractValueIsk:string; collateralExposureIsk:string; totalCommerceExposureIsk:string; marketOrdersSync:CommerceSyncOverview; marketHistorySync:CommerceSyncOverview; contractsSync:CommerceSyncOverview; }
...
export async function getMarketOrderHistoryDashboard():Promise<MarketOrderHistoryDashboard>{if(!inTauri())return {rows:[],summary:{newOrders:0,completedToday:0,completedThisWeek:0,cancelled:0,expired:0}};return invoke("get_market_order_history_dashboard");}
export async function markMarketOrderHistorySeen(characterId:number,orderId:number):Promise<void>{if(!inTauri())throw new Error(BROWSER_PREVIEW_ERROR);return invoke("mark_market_order_history_seen",{characterId,orderId});}
export async function markAllMarketOrderHistorySeen():Promise<number>{if(!inTauri())throw new Error(BROWSER_PREVIEW_ERROR);return invoke("mark_all_market_order_history_seen");}
export async function markContractActivitySeen(characterId:number,contractId:number):Promise<void>{if(!inTauri())throw new Error(BROWSER_PREVIEW_ERROR);return invoke("mark_contract_activity_seen",{characterId,contractId});}
export async function markAllContractActivitySeen():Promise<number>{if(!inTauri())throw new Error(BROWSER_PREVIEW_ERROR);return invoke("mark_all_contract_activity_seen");}
export async function getContractDashboard(filter:ContractFilter={}):Promise<ContractDashboard>{if(!inTauri())return {rows:[],summary:{outstandingContracts:0,assignedToMe:0,issuedByMe:0,inProgress:0,expiringSoon:0,completedOrFinished:0,totalCollateralExposureIsk:"0.00",outstandingRewardsIsk:"0.00",outstandingContractValueIsk:"0.00"},metrics:{outstandingContracts:0,completedToday:0,cancelledToday:0,expiredToday:0,contractsAwaitingAcceptance:0,newContractActivity:0}};return invoke("get_contract_dashboard",{filter});}
export async function getCommerceOverview(characterId:number|null=null,expiringDays:number|null=7):Promise<CommerceOverview>{if(!inTauri())return {remainingSellValueIsk:"0.00",remainingBuyCommitmentIsk:"0.00",outstandingContractValueIsk:"0.00",collateralExposureIsk:"0.00",totalCommerceExposureIsk:"0.00",marketOrdersSync:{status:"never",oldestSuccessAt:null,relevantCharacters:0,currentCharacters:0,errorCharacters:0},marketHistorySync:{status:"never",oldestSuccessAt:null,relevantCharacters:0,currentCharacters:0,errorCharacters:0},contractsSync:{status:"never",oldestSuccessAt:null,relevantCharacters:0,currentCharacters:0,errorCharacters:0}};return invoke("get_commerce_overview",{characterId,expiringDays});}
