...
## 1a. Domain Hierarchy (refined 003, extended 003.5, Operation Engine live in 004, Reservation Engine live in 005, Blueprint Engine live in 006, Production Requirement Engine live in 007, Static Data Import + real planner vertical slice in 008)
...
Static Data Import           (sibling to the above — owns official
                              reference-data ingestion: EVE types, groups,
                              categories, blueprint products/materials,
                              loaded from a local file, no network access)
```

As of Sprint 008, the Production Requirement Engine has a second, real
calculation path (`calculate_plan`) alongside its Sprint 007 demo-seeded
ledger: given an operation with a real build target (from Static Data
Import) and a real or assumed blueprint (from the Blueprint Engine or a
planning assumption), it recursively expands blueprint material lines —
ME-adjusted, run-rounded, cycle-protected — against real inventory
(demo `inventory_items` + real `manual_inventory_entries`) and
operation-specific reservations. See docs/REAL_PRODUCTION_PLANNER.md for
the full calculation rules, data ownership boundaries, and verification
procedure.
...
| **Production Requirement Engine** | Owns required inputs. Two calculation paths: the Sprint 007 demo-seeded ledger (`RequirementLine`/`RequirementSummary`, unchanged) and, as of Sprint 008, a real recursive calculator (`calculate_plan`) over imported static data + owned/assumed blueprints + real inventory — ME-adjusted, cycle-protected, deterministic. Never touches inventory, reservation, operation-lifecycle, or blueprint tables directly (only reads them). Mutation methods (declare requirement, adjust quantity) remain "architecture-only" stubs. | 007 (demo reads), 008 (real calculation) |
| **Static Data Import** | Owns official reference-data ingestion: EVE types, groups, categories, blueprint products and materials, loaded from a local file/directory the user selects. No network access, ever. **Live as of Sprint 008** — `src-tauri/src/staticdata/` (no provider/engine trait indirection: exactly one data source, a local file, so the extra abstraction layer was judged unnecessary ceremony). Two import paths as of Sprint 008.2 — the CSV-derivative sample fixture (verified end-to-end) and CCP's official JSON Lines SDE export (`jsonl.rs`, tolerant field-name matching, unverified against a real export in this environment) — both funneling through one shared transactional upsert function (`apply_parsed_import`) so there is no duplicated import logic between them. Never touches operations, inventory, reservations, blueprints, or settings. | 008, 008.2 |
...
1h. **007** — Production Requirement Engine Foundation: migration 0007 (`production_requirements`, `production_requirement_groups`, `production_requirement_sources`); real `production/` module with live reads (lines, detail, categories, summary, shortages, cross-operation critical bottlenecks, per-operation and build-target breakdown). Production page becomes live (requirement tree, category filters, shortage report, critical bottlenecks); Mission Control gains a Production Readiness panel; the Operations Workspace gains a live Production Requirements section (per-category coverage bars). Mutations (declare requirement, adjust quantity) remain architecture-only; no ESI, SDE import, shopping logic, manufacturing jobs, production scheduling, or Decision Engine rules. *(done)*
1i. **008** — Real Production Planner vertical slice: migration 0008 (`sde_imports`, `eve_categories`, `eve_groups`, `eve_types`, `blueprint_products`, `blueprint_materials`, `operation_build_targets`, `manual_inventory_entries`, `production_plan_snapshots`, additive `operations.is_demo`); new `staticdata/` module (CSV-derivative SDE import, transactional, repeatable); `OperationEngine::create_operation` upgraded from stub to real; `production::calculate_plan` — real recursive, ME-adjusted, cycle-protected requirement calculation; `inventory/` gains real manual-inventory CRUD. Settings page goes live (Static Data Import); Operations Workspace gains a real New Operation flow; Production page gains a Real Production Plan tree with CSV/Markdown/clipboard export; Mission Control gains a Real Operations Readiness panel. See docs/REAL_PRODUCTION_PLANNER.md. No ESI, no native SDE YAML parsing, no manufacturing jobs, no scheduling, no Decision Engine rules. *(done)*
1j. **008.2** — Official CCP JSON Lines SDE import (`staticdata/jsonl.rs`, tolerant field-name matching, unverified against a real export in this environment — see docs/REAL_PRODUCTION_PLANNER.md); shared import transaction logic extracted (`apply_parsed_import`) so both the sample-fixture and official paths use one code path, not two; `LeafTotal` gains `groupName`/`categoryName` (populated after recursive expansion finishes, never touching the expansion/ME/run-rounding logic itself); Inventory gains a Paste Inventory bulk workflow (tolerant tab/comma/space parsing, preview, duplicate merging, exact-name matching); Production page gains a Flattened Materials view alongside the Build Tree. No migration — reuses migration 0008's tables entirely. *(this sprint)*
