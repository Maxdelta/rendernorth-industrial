...
pub const FEATURE_REQUEST_URL: &str =
    "https://github.com/Maxdelta/rendernorth-industrial/issues/new?labels=enhancement";
...
pub const OPEN_BETA_STATUS: &str = "Open Beta 0.1.6";
...
pub fn inspect_sde(path: &str) -> SdeInspection {
    ...
    if !directory.exists() || !directory.is_dir() || fs::read_dir(directory).is_err() {
        return SdeInspection {
            path: trimmed.into(),
            valid: false,
            status: "inaccessible".into(),
            source_build: None,
            files: required_file_status(directory),
            error: Some("The selected folder cannot be accessed or no longer exists.".into()),
        };
    }
    let files: Vec<SdeFileStatus> = REQUIRED_SDE_FILES
        .iter()
        .map(|name| SdeFileStatus {
            name: (*name).into(),
            present: directory.join(name).is_file(),
        })
        .collect();
    ...
    let invalid_jsonl = if missing.is_empty() {
        REQUIRED_SDE_FILES
            .iter()
            .any(|name| !first_record_is_json(&directory.join(name)))
    } else {
        false
    };
    let (status, error) = if present_count == 0 {
        (
            "wrong_folder",
            Some("This does not appear to be the supported CCP JSONL static-data folder.".into()),
        )
    } else if !missing.is_empty() {
        (
            "incomplete_extraction",
            Some(format!(
                "Incomplete extraction. Missing required files: {}",
                missing.join(", ")
            )),
        )
...
pub fn authentication_info(conn: &Connection) -> Result<AuthenticationInfo, String> {
    let custom: Option<String> = conn
        .query_row(
            "SELECT value FROM app_meta WHERE key='esi_custom_client_id' AND trim(value)<>''",
            [],
            |row| row.get(0),
        )
        .optional()
    ...
    Ok(match custom {
        Some(client_id) => AuthenticationInfo {
            mode: "custom".into(),
            client_id,
            application_name: "Custom CCP Application".into(),
        },
        None => AuthenticationInfo {
            mode: "official".into(),
            client_id: OFFICIAL_CLIENT_ID.into(),
            application_name: "Official RenderNorth Industrial".into(),
        },
    })
}
...
pub fn restore_official_authentication(conn: &Connection) -> Result<AuthenticationInfo, String> {
    conn.execute("DELETE FROM app_meta WHERE key='esi_custom_client_id'", [])
        .map_err(|error| error.to_string())?;
    authentication_info(conn)
}
...
pub fn pick_sde_archive(app: &AppHandle) -> Option<String> {
    app.dialog()
        .file()
        .add_filter("CCP static-data ZIP", &["zip"])
        .blocking_pick_file()
        .and_then(|path| path.into_path().ok())
        .map(|path| path.to_string_lossy().to_string())
}
...
pub fn about_info(conn: &Connection) -> Result<AboutInfo, String> {
    ...
    Ok(AboutInfo {
        ...
        build: option_env!("RENDERNORTH_BUILD_UNIX")
            .unwrap_or("unknown")
            .into(),
        git_commit: option_env!("RENDERNORTH_GIT_COMMIT")
            .unwrap_or("unknown")
            .into(),
        database_version: format!("SQLite {}", rusqlite::version()),
        migration_version,
        rust_version: option_env!("RENDERNORTH_RUST_VERSION")
            .unwrap_or("unknown")
            .into(),
        website: WEBSITE_URL.into(),
        github: GITHUB_URL.into(),
        issues: ISSUES_URL.into(),
        support: SUPPORT_URL.into(),
        discord_invite: DISCORD_INVITE_URL.into(),
        setup_guide: SETUP_GUIDE_URL.into(),
        release_status: OPEN_BETA_STATUS.into(),
...
fn diagnostics_value(conn: &Connection) -> Result<Value, String> {
    ...
    Ok(json!({
        ...
        "market": market,
        "commerce": crate::commerce::safe_diagnostics(conn)?,
...
pub fn export_diagnostics(app: &AppHandle, conn: &Connection) -> Result<Option<String>, String> {
    let suggested = format!(
        "rendernorth-industrial-diagnostics-{}.json",
    ...
    fs::write(&path, diagnostics_json(conn)?)
...
mod tests {
    ...
    fn official_authentication_is_the_default_public_client() {
        ...
        assert_eq!(
            authentication_info(&conn).unwrap().client_id,
            "custom-client-id-123456"
        );
        assert_eq!(
            restore_official_authentication(&conn).unwrap().mode,
            "official"
        );
    }
    ...
    fn official_download_and_open_beta_help_values_are_allowlisted() {
        ...
        assert_eq!(OPEN_BETA_STATUS, "Open Beta 0.1.6");
        ...
        assert_eq!(
            GITHUB_URL,
            "https://github.com/Maxdelta/rendernorth-industrial"
        );
        ...
        assert!(!external_url_allowed(
            "https://github.com/Maxdelta/rendernorth-industrial/releases.evil.example/file.exe"
        ));
    ...
    fn sde_validation_distinguishes_archive_wrong_folder_and_unsupported_format() {
        ...
        assert_eq!(
            inspect_sde(archive.to_str().unwrap()).status,
            "archive_selected"
        );
        assert_eq!(inspect_sde(root.to_str().unwrap()).status, "wrong_folder");
        for file in REQUIRED_SDE_FILES {
            fs::write(root.join(file), "not-json\n").unwrap();
        }
        assert_eq!(
            inspect_sde(root.to_str().unwrap()).status,
            "unsupported_format"
        );
    ...
    fn diagnostics_are_explicitly_secret_free() {
        ...
        assert!(report.contains("No order rows, contract contents"));
    }
}
